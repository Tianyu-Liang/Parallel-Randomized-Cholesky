...
template <typename type_int, typename type_data>
void remove_last_row_and_column(sparse_matrix<type_int, type_data> &A) {
...
template <typename type_int, typename type_data>
__global__ void input_check_last(sparse_matrix_device<type_int, type_data> &spmat, type_int *csr_row_ptr)
{
    ...
    int block_size = blockDim.x;
    int lane_id = threadIdx.x % block_size;
    type_int n = spmat.num_cols;

    // determine row start
    for(type_int index = bid; index < n; index += gap)
    {
        
        type_int start_local = spmat.col_ptrs[index];
        type_int size_local = spmat.col_ptrs[index + 1] - start_local;
        // determine start location within each row
        for(type_int inner = lane_id; inner < size_local; inner += block_size)
        {
            // can only happen to 1 thread
            if (spmat.row_indices[start_local + inner] == n)
            {
                csr_row_ptr[index + 1]--;
            }
...
// go from prefix sum to before
template <typename type_int, typename type_data>
__global__ void revert_from_prefix(type_int *csr_row_ptr, type_int *ref, type_int n)
{
    int tid = threadIdx.x;
    int bid = blockIdx.x;
    int block_size = blockDim.x;
    ...
    for(type_int index = bid * block_size + tid + 1; index < n; index += gap)
    {
        csr_row_ptr[index] = ref[index] - ref[index - 1];
    }

}

// copy the original input into a new location with the last row removed
template <typename type_int, typename type_data>
__global__ void input_lap_construct_csr(sparse_matrix_device<type_int, type_data> &spmat, type_int *csr_row_ptr, type_int *csr_col_ind, type_data *csr_val)
{
    ...
    for(type_int index = bid; index < n; index += gap)
    {
        type_int size_local = csr_row_ptr[index + 1] - csr_row_ptr[index];
        type_int start_local = csr_row_ptr[index];
        // determine start location within each row
        for(type_int inner = lane_id; inner < size_local; inner += block_size)
        {

            csr_col_ind[csr_row_ptr[index] + inner] = spmat.row_indices[spmat.col_ptrs[index] + inner];
            csr_val[csr_row_ptr[index] + inner] = spmat.values[spmat.col_ptrs[index] + inner];
...
// driver function for trimming input
template <typename type_int, typename type_data>
void trim_input_laplacian_device(sparse_matrix_device<type_int, type_data> &spmat)
{
    spmat.num_cols = spmat.num_cols - 1;
    spmat.num_rows = spmat.num_rows - 1;
    type_int num_cols = spmat.num_cols;
    type_int *csr_rowptr_device;
    cudaMalloc((void**)&csr_rowptr_device, (num_cols + 1) * sizeof(type_int));
    cudaMemcpy(csr_rowptr_device, spmat.col_ptrs, num_cols + 1, cudaMemcpyDeviceToDevice);
    revert_from_prefix<type_int, type_data><<<256, 128>>>(csr_rowptr_device, spmat.col_ptrs, num_cols + 1);
    cudaDeviceSynchronize();


    input_check_last<type_int, type_data><<<1024, 32>>>(spmat, csr_rowptr_device);
    cudaDeviceSynchronize();
    ...
    input_lap_construct_csr<type_int, type_data><<<2048, 32>>>(spmat, 
        csr_rowptr_device, csr_col_ind_device, csr_val_device);
    cudaDeviceSynchronize();

    cudaFree(spmat.row_indices);
    cudaFree(spmat.col_ptrs);
    cudaFree(spmat.values);

    spmat.row_indices = csr_col_ind_device;
    spmat.col_ptrs = csr_rowptr_device;
    spmat.values = csr_val_device;
...
void factorization_driver(sparse_matrix_processor<type_int, type_data> &processor, int num_blocks, bool check_solve, double tolerance)
{
    ...
    std::cout << "initial preprocessing time (mostly allocation and copy time, not part of the actual algorithm): " << duration.count() << " milliseconds" << std::endl;
    ...
    if(check_solve)
    {
        ...
        trim_input_laplacian_device<type_int, type_data>(spmat_device);
        // trim the last row and column of mat

        // remove_last_row_and_column(processor.mat);
