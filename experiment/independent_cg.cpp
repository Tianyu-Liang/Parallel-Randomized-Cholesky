...
void example_pcg_solver(custom_space::sparse_matrix<type_int, type_data> &A, custom_space::sparse_matrix<type_int, type_data> &M, bool is_graph) {
    ...
    if(!is_graph)
    {
        // NO NEED TO REMOVE FROM M HERE BECAUSE IT'S CREATED FROM MATLAB AND SHOULD ALREADY HAVE IT REMOVED
    ...
    descr_M.mode = SPARSE_FILL_MODE_LOWER;
    ...
    while (true) 
    {
        ...
        if (RCI_request == 0) 
        {
            // mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE, 1.0, A_handle, descr_A, x.data(), 0, temp_for_pre);
            // cblas_daxpy(n, -1, b.data(), 1, temp_for_pre, 1);
            
            // if(cblas_dnrm2(n, temp_for_pre, 1) / cblas_dnrm2(n, b.data(), 1) < sqrt(dpar[0]))
            // {
        ...
        {
            mkl_sparse_d_trsv(SPARSE_OPERATION_NON_TRANSPOSE, 1.0, M_handle, descr_M, &tmp[2 * n], temp_for_pre);

            mkl_sparse_d_trsv(SPARSE_OPERATION_TRANSPOSE, 1.0, MT_handle, descr_M, temp_for_pre, &tmp[3 * n]);
        }
        else 
        {
...
template <typename type_int, typename type_data>
custom_space::sparse_matrix<type_int, type_data> read_in_factor(char *input_char)
{
    std::string input(input_char);
    std::ifstream input_stream(input);

    if (!input_stream) {
        std::cerr << "Error opening file " << input_char << std::endl;
        exit(0);
    }
    ...
    size_t nnz = input_triplet.vals.size();

    // Convert the triplet data to CSR format.
    // Allocate CSR arrays with zero-based indexing.
    std::vector<type_int> ia(input_triplet.nrows + 1, 0);
    std::vector<type_int> ja(nnz);
    std::vector<type_data> a(nnz);

    // Count the number of nonzeros in each row.
    // Since the triplet row indices are zero-based, we increment ia[row+1].
    for (size_t i = 0; i < nnz; i++) {
        ia[input_triplet.rows[i] + 1]++;
    }

    // Compute the cumulative sum to obtain the row pointer array.
    for (type_int i = 1; i <= input_triplet.nrows; i++) {
        ia[i] += ia[i - 1];
    }

    // Copy of ia to use as a pointer into each row.
    std::vector<type_int> row_ptr = ia;

    // Fill the CSR column index and value arrays.
    for (size_t i = 0; i < nnz; i++) {
        type_int row = input_triplet.rows[i];
        type_int pos = row_ptr[row]++;
        ja[pos] = input_triplet.cols[i];
        a[pos] = input_triplet.vals[i];

        // if(input_triplet.rows[i] == 100)
        // {
        //     printf("cols: %d\n", input_triplet.cols[i]);
        // }
    }

    return custom_space::sparse_matrix(input_triplet.nrows, input_triplet.ncols, std::move(a), std::move(ja), std::move(ia));
}
...
{
    printf("problem: %s\n", argv[1]);
    sparse_matrix_processor<int, double> processor(argv[1]);
