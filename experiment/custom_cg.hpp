...
template <typename type_int, typename type_data>
void example_pcg_solver(custom_space::sparse_matrix<type_int, type_data> &A, custom_space::sparse_matrix<type_int, type_data> &M, type_data *diagonal, bool is_graph, int max_iter = 1000, double rel_tol = 1e-7, char* rhsfile = nullptr) {
    ...
    if(rhsfile != nullptr)
    {
        // Read RHS from file (HyPre IJ vector format: a header line, then "idx val" per line, idx in
        // ascending order). Lets AC solve the EXACT same b = A*x that HyPre/the reference use.
        std::ifstream rin(rhsfile);
        if(!rin) { printf("ERROR: cannot open rhsfile %s\n", rhsfile); assert(false); }
        std::string header;
        std::getline(rin, header);              // skip the "ilower iupper" header line
        type_int idx; type_data val;
        for(type_int i = 0; i < n; i++)
        {
            if(!(rin >> idx >> val))
            {
                printf("ERROR: rhsfile %s has fewer than n=%d entries. (rhsfile is intended for graph "
                       "mode, where n is the full size; physics mode removes a row so sizes won't match.)\n",
                       rhsfile, (int)n);
                assert(false);
            }
            b[i] = val;
    ...
    {
        // Zero-sum RANDOM RHS. WARNING: only valid for CONNECTED graphs. For a DISCONNECTED graph this
        // is generally NOT in the image of A (each component needs its own zero-sum component), so the
        // solve will not converge -- supply a file RHS (b = A*x, always in the image) instead.
        generate_zero_sum_vector(b.data(), n, 0);
    }
    ...
    int counter = 0;
    double norm_rhs = cblas_dnrm2(n, b.data(), 1);   // ||b||, for the relative-residual stopping test
    while (true)
    {
        ...
        else if (RCI_request == 2) 
        {
            ...
            //     continue;
            // }
            // Relative TRUE-residual stop: keep going while ||b-Ax||/||b|| > rel_tol.
            // sqrt(dpar[4]) = ||b-Ax|| is maintained by MKL each iteration (free; no extra SpMV).
            // (Previously compared ||b-Ax|| to sqrt(rel_tol) -- absolute and sqrt'd, ~100x too loose.)
            if(sqrt(dpar[4]) > rel_tol * norm_rhs)
            {
                continue;
            }
            else
    ...
    // norm_rhs (||b||) already computed before the loop; b is untouched until the next line.
