...
void factorization_driver(sparse_matrix_processor<type_int, type_data> &processor, type_int num_threads, char* path, bool is_graph)
{
    ...
    */     
    ...
    // 1. triangular solve longest DAG path

    std::vector<size_t> max_path_dp(spmat.num_cols, 1);
    std::vector<int> track_path(spmat.num_cols, -1);
    for(size_t i = 0; i < spmat.num_cols; i++)
    {
        ...
        {
            if(max_path_dp[i] + 1 > max_path_dp[csr_col_ind_host[j]])
            {
                track_path[csr_col_ind_host[j]] = i;
    ...
    //writeVectorToFile(track_path, "check-path");
    // 2. verify the max path length and find the path associated with it
    auto max_path = std::max_element(max_path_dp.begin(), max_path_dp.end());
    type_int index_of_max = std::distance(max_path_dp.begin(), max_path);
    type_int verify_len = 0;
    type_int path_begin = -1;
    std::stack<type_int> stack;
    while(index_of_max != -1)
    {
        stack.push(index_of_max);
        if(track_path[index_of_max] == -1)
        {
            path_begin = index_of_max;
        }
        
        assert(track_path[index_of_max] < index_of_max);
        index_of_max = track_path[index_of_max];
        verify_len++;
    }
    printf("verify len: %d\n", verify_len);


    // 3. traverse the path to make sure it's valid
    type_int pt = stack.top();
    type_int path_count = 1;
    stack.pop();
    while(!stack.empty())
    {
        ...
        size_t left_end = csr_rowptr_host[pt];
        size_t right_end = csr_rowptr_host[pt + 1];
        bool found = false;
        for(size_t j = left_end + 1; j < right_end && !found; j++)
        {
            found = (csr_col_ind_host[j] == stack.top());
        }
        assert(found);
    ...
    printf("path len starting from %d: %d\n", path_begin, path_count);
